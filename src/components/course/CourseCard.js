...
export default function CourseCard({ course }) {
  return (
    <Link
      ...
      className="border-dark-1/5 dark:border-light-1/5 group dark:bg-dark-1 bg-light-1 flex flex-1 flex-col justify-between rounded-xl border transition-all select-none"
    >
      {/* Cover */}
      <div>
        {course.cover_url ? (
      ...
      <div className="flex h-full flex-col justify-between">
        ...
        <div className="flex flex-col gap-2 p-3 sm:p-4">
          ...
          <div className="text-dark-1/60 dark:text-light-1/60 line-clamp-5 text-[10px] sm:line-clamp-4 sm:text-xs/relaxed">
            {course.short_description ?? "توضیحی برای این دوره ثبت نشده است."}
          </div>
        </div>

        {/* Bottom */}
        <div className="border-dark-1/10 dark:border-light-1/10 flex flex-wrap items-center justify-between gap-2 border-t p-4">

...
const menuItems = [
  ...
  { title: "تنظیمات", href: "/profile/settings", icon: LuSettings },
];
        ...
        {visibleItems.map((item) => {
          ...
          return (
            <li key={item.href} className="shrink-0">
              <Link
                href={item.href}
                className={`flex items-center gap-3 rounded-xl px-3 py-3 text-sm font-medium whitespace-nowrap transition-all ${isActive ? "bg-primary/10 text-primary" : "text-dark/70 dark:text-light/70 hover:bg-primary/5 hover:text-primary"}`}
              >
